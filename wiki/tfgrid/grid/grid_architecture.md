...
The ThreeFold_Grid needs hardware/servers to function. Servers of all shapes and sizes can be added to the grid by anyone, anywhere in the world. The production of Internet Capacity on the Threefold Grid is called Farming and people who add these servers to the grid are called ThreeFold Farmers ("Farmers"). Being a decentralized process and involving the participation of the community to expand the grid, Farmers are rewarded by the means of TFT. 

Farmers download the Zero-OS operating system and boot their servers themselves. Once booted, these servers become 3Nodes. The 3Nodes will register themselves in a database called the TF Directory. Once registered in the TF Directory, the capacity of the 3Nodes will become available on the ThreeFold Grid Explorer. Also, given the autonomous nature of the ThreeFold Grid, there is no need for any intermediaries between the user and 3Nodes. 
